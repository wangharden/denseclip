...
from fewshot.stage_a2 import score_with_dual_prototype


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage A2 dual-prototype baseline on official MVTec structure.")
    ...
    parser.add_argument("--output-dir", default="outputs/stage_a2")
    parser.add_argument("--image-size", type=int, default=320)
    parser.add_argument("--support-normal-k", type=int, default=8)
    parser.add_argument("--support-defect-k", type=int, default=4)
    parser.add_argument("--score-mode", default=SCORE_MODE_DEFECT_MINUS_NORMAL, choices=SCORE_MODES)
        ...
        json.dumps(
            {
                ...
                "num_defect_references": num_defect_references,
            },
            indent=2,
        ),
        encoding="utf-8",
...
def main() -> None:
    args = parse_args()
    if args.support_defect_k < 1 and not args.split_manifest:
        raise ValueError("Stage A2 requires --support-defect-k >= 1 when no split manifest is provided.")
    set_seed(args.seed)
    ...
    if not split.support_defect:
        raise ValueError("Stage A2 requires a split manifest with at least one defect support sample.")
    ...
    defect_prototype = bank.defect_reference
    if defect_prototype is None:
        raise ValueError("Stage A2 requires defect references.")

    prototype_path = output_root / "stage_a2_prototype.pt"
    torch.save(
        {
            "normal_prototype": normal_prototype.detach().cpu(),
            "defect_prototype": defect_prototype.detach().cpu(),
            "normal_reference": normal_prototype.detach().cpu(),
            "defect_reference": defect_prototype.detach().cpu(),
            "support_normal_paths": bank.normal_support_paths,
            "support_defect_paths": bank.defect_support_paths,
            "prototype_family": args.prototype_family,
            "num_prototypes": args.num_prototypes,
            "reference_topk": args.reference_topk,
            "config": vars(args),
    ...
    for batch in loader:
        ...
        outputs = score_with_dual_prototype(
            encoder=encoder,
            images=images,
            normal_prototype=normal_prototype.to(device),
            defect_prototype=defect_prototype.to(device),
            image_size=args.image_size,
    ...
    save_outputs(
        ...
        num_defect_references=int(defect_prototype.shape[0]) if defect_prototype.ndim == 2 else 1,
    )
